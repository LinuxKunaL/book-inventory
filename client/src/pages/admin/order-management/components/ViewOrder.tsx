...
function ViewOrder() {
  ...
  useEffect(() => {
    if (isViewModalOpen.data) {
      setUpdateStatus(isViewModalOpen.data.status);
    }
        ...
        <div className="space-y-3 sm:space-y-6 p-2 sm:p-5">
          <div className="mb-4 sm:mb-6 flex justify-between">
            <div>
              <h3 className="font-semibold tracking-tight text-xl">
                Order Details{" "}
                <Badge
                  color={getStatusColor(isViewModalOpen.data.status)}
                  className="capitalize"
                  size={"3"}
                >
                  {isViewModalOpen.data.status}
                </Badge>
              </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <div>
              <p className="text-gray-600 dark:text-gray-400">
                {isViewModalOpen?.data?.createdBy?.fullname}
              </p>
            </div>
            <div>
              <p className="text-gray-600 dark:text-gray-400">
                {isViewModalOpen.data.createdBy.email}
              </p>
            </div>
          </div>
          <div className="gap-2">
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Select
              ...
              value={updateStatus}
              onChange={(value: any) => setUpdateStatus(value)}
              BgColor="bg-gray-800"
